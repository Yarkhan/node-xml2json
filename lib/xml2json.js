...
function endElement (state, name) {
  if (state.currentObject[textNodeName(state)]) {
    ...
    state.currentObject[textNodeName(state)] = state.options.coerce
      ? coerce(state.currentObject[textNodeName(state)], name, state)
      : state.currentObject[textNodeName(state)]
  }
...
module.exports = function (xml, _options = defaultOptions) {
  ...
  let json = JSON.stringify(state.currentObject)
